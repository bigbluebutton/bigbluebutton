...
func ConnectionHandler(w http.ResponseWriter, r *http.Request) {
	...
	var thisConnection = common.BrowserConnection{
		...
		FromBrowserToHasuraRateLimiter:     rate.NewLimiter(rate.Every(600*time.Millisecond), cfg.Server.MaxConnectionQueriesPerMinute),
		FromBrowserToGqlActionsChannel:     common.NewSafeChannelByte(bufferSize),
		FromBrowserToGqlActionsRateLimiter: rate.NewLimiter(rate.Every(600*time.Millisecond), cfg.Server.MaxConnectionMutationsPerMinute),
