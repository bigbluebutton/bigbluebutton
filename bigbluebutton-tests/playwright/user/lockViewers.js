...
export class LockViewers extends MultiUsers {
  ...
  async lockSendPrivateChatMessages() {
    ...
    const lastUserItemLocator = this.userPage.page.locatorByIndex(e.userListItem, -1);
    await this.userPage.clickOnLocator(lastUserItemLocator);
    const startPrivateChatButton = this.userPage.page.locatorByIndex(e.startPrivateChat, -1);
  ...
  async lockSeeOtherViewersAnnotations() {
    ...
    await this.modPage.page.waitForTimeout(1000);   // timeout to ensure that the userPage presentation is zoomed in stabilized
    ...
    await this.modPage.page.locator(e.chatButton).hover();
    await this.userPage2.page.locator(e.chatButton).hover(); // ensure userPage cursor won't be visible on the screenshot
    await this.modPage.page.waitForTimeout(1000);  // expected timeout for cursor indicator to disappear
  ...
  async lockSeeOtherViewersCursor() {
    ...
    await this.modPage.page.locator(e.chatButton).hover(); // ensure modPage cursor WILL NOT be visible on the screenshot
