...
export class CustomParameters extends MultiUsers {
  ...
  async showSessionDetailsOnJoin() {
    ...
    // avoid throwing error due to both modals being in the DOM
    const audioModalCloseButton = await this.modPage.getVisibleLocator(e.closeModal);
  ...
  async customStyle() {
    ...
    const resp = await this.modPage.page.evaluate(
      (elem) => (document.querySelectorAll(elem)[0] as HTMLElement)?.offsetHeight === 0,
      e.presentationTitle,
    );
  ...
  async hidePresentationOnJoinReturnFromBreakouts() {
    ...
    if (!this.userPage.context) throw new Error('userPage context is not defined');
    const initialContextPagesCount = this.userPage.context?.pages().length || 0;
    await this.userPage.waitAndClick(e.modalConfirmButton);
    await expect
      .poll(
        async () => {
          return this.userPage.context?.pages().length || 0;
        },
        {
          message: 'a new page should be opened for the breakout room',
          timeout: ELEMENT_WAIT_TIME,
        },
      )
      .toBeGreaterThan(initialContextPagesCount);
  ...
  async multiUserPenOnly() {
    ...
    await this.userPage.hasElement(e.wbToolbar, 'should display the whiteboard toolbar');
  ...
  async logoutURLTest() {
    ...
    await this.modPage.hasElement(e.meetingEndedModal, 'should display the meeting ended modal');
  ...
  async enforceParticipantsAndChatOnly() {
    ...
    await this.modPage.hasElement(e.chatMessages, 'should display the chat messages');
    await this.modPage.hasElement(e.userListContent, 'should display the user list content');
  ...
  async enforceMediaOnly() {
    ...
    await this.modPage.hasElement(e.joinVideo, 'should display the join video button');
    await this.modPage.hasElement(e.startScreenSharing, 'should display the start screen sharing button');

    await this.modPage.wasRemoved(e.chatMessages, 'should not display the chat messages');
    await this.modPage.wasRemoved(e.userListContent, 'should not display the user list content');
    await this.modPage.page.waitForTimeout(1000);
