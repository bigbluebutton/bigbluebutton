...
export class Chat extends MultiUsers {
  ...
  async emojiCopyChat() {
    ...
    if (!emojiPickerEnabled) {
      ...
      return this.modPage.wasRemoved(
        e.emojiPickerButton,
  ...
  async closePrivateChat() {
    ...
    await this.modPage.hasElement(
      e.closePrivateChat,
      'the private chat should display the close private chat element when opened'
    );
  ...
  async autoConvertEmojiSendPrivateChat() {
    ...
    await this.userPage.hasElement(
      e.hidePrivateChat,
      'should display the hide private chat element for the attendee when the private chat is opened'
    );
    // check sent messages
