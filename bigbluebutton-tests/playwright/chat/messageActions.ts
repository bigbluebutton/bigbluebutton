...
export class MessageActions extends Chat {
  async editMessageFromToolbarButton() {
    ...
    await this.modPage.hasElementCount(
      ...
      'should keep displaying only one message on the public chat',
    );
    await this.modPage.hasElement(
  ...
  async editMessageFromArrowUp() {
    ...
    await expect(this.modPage.page.locator(e.chatBox), 'should not display any text in the chat box').toHaveValue('');
  }
  ...
  async ableToEditOwnMessage() {
    ...
    await this.modPage.hasElement(
      e.chatUserMessageText,
      'should display the message sent by the moderator on its public chat',
    );
    await this.userPage.hasElement(
      e.chatUserMessageText,
      'should display the message sent by the moderator on the user public chat',
    );
    await checkLastMessageSent(this.modPage, e.message);
    ...
    await expect(
      lastMessageItem.locator(e.replyMessageButton),
      'should display the reply message button when hovering the mod message',
    ).toBeVisible();
    await expect(
      lastMessageItem.locator(e.reactMessageButton),
      'should display the react message button when hovering the mod message',
    ).toBeVisible();
    await expect(
      lastMessageItem.locator(e.editMessageButton),
    ...
    await this.userPage.hasElementCount(
      e.chatUserMessageText,
      2,
      'should display both messages sent on the mod public chat',
    );
    await this.modPage.hasElementCount(
      e.chatUserMessageText,
      2,
      'should display both messages sent on the user public chat',
    );
    ...
    await expect(
      lastMessageSent.replyMessageButton,
      'should display the reply message button when hovering a user message',
    ).toBeVisible();
    await expect(
      lastMessageSent.reactMessageButton,
      'should display the react message button when hovering a user message',
    ).toBeVisible();
    await expect(
      lastMessageSent.editMessageButton,
      'should not display the edit message button when hovering a user message',
    ).not.toBeVisible();
  }
  ...
  async deleteOwnMessage() {
    ...
    await this.modPage.hasElementCount(
      e.chatMessageItem,
      initialMessageItemsCount,
      'should keep displaying the deleted message item on the public chat',
    );
    await this.modPage.wasRemoved(
      e.messageReactionItem,
      'should remove the message reaction item from the deleted message',
    );
    await lastMessageItem.hover();
    await expect(
      lastMessageItem.locator(e.messageToolbar),
      'should not display the message toolbar when hovering a deleted message',
    ).not.toBeVisible();
  }
  ...
  async deleteAnotherUserMessage() {
    ...
    await this.userPage.hasElement(
      e.chatUserMessageText,
      'should display the message sent by the user on its public chat',
    );
    await checkLastMessageSent(this.modPage, e.message);
    const initialMessageItemsCount = await this.modPage.getSelectorCount(e.chatMessageItem);
    // hover and react message
    ...
    await this.modPage.hasElement(
      e.chatUserMessageText,
      'should display the message sent by the mod2 on the public chat',
    );
    await checkLastMessageSent(this.modPage, e.message);
    await lastMessageItem.hover();
    await expect(
    ...
    await this.modPage.waitAndClick(e.confirmDeleteChatMessageButton);
    // check deleted message
    const lastMessageItem2 = this.modPage.page.locator(e.chatMessageItem).last();
    await expect(lastMessageItem2, 'should display the message deleted label after deleting a message').toContainText(
      `This message has been deleted by ${this.modPage.username}`,
    );
    await this.modPage.wasRemoved(e.chatUserMessageText, 'should remove the message content from the public chat');
    await this.modPage.hasElementCount(
      e.chatMessageItem,
      initialMessageItemsCount + 1,
      'should keep displaying the mod2 deleted message item on the public chat',
    );
  }
  ...
  async breakoutsModDelete() {
    ...
    await this.userPage.hasElement(
      e.alreadyConnected,
      'should display the element alreadyConnected when user join',
    ...
    const lastMessageItemBreakoutUser = breakoutUserPage.page.locator(e.chatMessageItem).last();
    await lastMessageItemBreakoutUser.hover();
    await expect(
      lastMessageItemBreakoutUser.locator(e.messageToolbar),
      'should display the message toolbar when hovering a user message',
    ).toBeVisible();
    await expect(
      lastMessageItemBreakoutUser.locator(e.replyMessageButton),
      'should display the reply message button when hovering a user message',
    ).toBeVisible();
    await expect(
      lastMessageItemBreakoutUser.locator(e.reactMessageButton),
      'should display the react message button when hovering a user message',
    ).toBeVisible();
    await expect(
      lastMessageItemBreakoutUser.locator(e.editMessageButton),
      'should not display the edit message button when hovering a user message',
    ).not.toBeVisible();
    // check if mod can delete own message
    const lastMessageItemBreakoutMod = breakoutModPage.page.locator(e.chatMessageItem).last();
    await lastMessageItemBreakoutMod.hover();
    await expect(
      lastMessageItemBreakoutMod.locator(e.messageToolbar),
      'should display the message toolbar when hovering a message',
    ).toBeVisible();
    await expect(
      lastMessageItemBreakoutMod.locator(e.replyMessageButton),
      'should display the reply message button on the toolbar',
    ).toBeVisible();
    await expect(
      lastMessageItemBreakoutMod.locator(e.reactMessageButton),
      'should display the react message button on the toolbar',
    ).toBeVisible();
    await expect(
      lastMessageItemBreakoutMod.locator(e.editMessageButton),
      'should display the edit message button on the toolbar',
    ).toBeVisible();
    ...
    // check deleted message
    const lastMessageItemMod = await breakoutModPage.page.locator(e.chatMessageItem).last();
    await expect(
      lastMessageItemMod,
      'should display the message deleted label after deleting a message for the mod',
    ).toContainText(`This message has been deleted by ${this.modPage.username}`);
    const lastMessageItemUser = await breakoutUserPage.page.locator(e.chatMessageItem).last();
    await expect(
      lastMessageItemUser,
      'should display the message deleted label after deleting a message for the viewer',
    ).toContainText(`This message has been deleted by ${this.modPage.username}`);
  }
  ...
  async replyMessage() {
    ...
    const closeReplyBtn = this.modPage.page.locator(e.closeChatReplyIntentionButton);
    await this.modPage.hasElement(
    ...
    await this.modPage.hasText(
      e.chatReplyIntentionContainerContent,
      e.message,
      'should display the replying message content in the container',
    );
    // reply the message
    await this.modPage.fill(e.chatBox, e.message2);
    await this.modPage.waitAndClick(e.sendButton);
  ...
  async cancelReplyMessage() {
    ...
    await this.modPage.wasRemoved(
      e.chatReplyIntentionContainerContent,
      'should remove the reply intention container content (by clicking on the cancel button)',
    );
    await expect(
      closeReplyBtn,
    ...
    await this.modPage.hasElementCount(
      e.chatUserMessageText,
      initialMessagesCount,
      'should keep displaying the same number of messages on the public chat',
    );
  ...
  async scrollToRepliedMessage() {
    ...
    await this.modPage.hasElement(
      e.chatUserMessageText,
      'should display the first message sent by the moderator on the public chat',
    );
    const targetMessageMod = this.modPage.page.locator(e.chatMessageItem, { hasText: e.message });
    // send many messages
    ...
    for (let i = 1; i <= MESSAGES_COUNT; i += 1) {
      ...
      await this.modPage.hasElementCount(
        e.chatUserMessageText,
        i + 1,
        'should display the last message sent by the moderator on the public chat',
      );
    }
    await expect(
      targetMessageMod,
    ...
    await expect(
      targetMessageMod,
      'should display the first message in the preview window after scrolling to it',
    ).toBeInViewport();
    // reply to message
    await targetMessageMod.hover();
    await expect(
      targetMessageMod.locator(e.messageToolbar),
      'should display the message toolbar when hovering a message',
    ).toBeVisible();
    await this.modPage.waitAndClick(e.replyMessageButton);
    await this.modPage.hasElement(
      e.chatReplyIntentionContainerContent,
      'should display the chat reply intention when replying to a message',
    );
    ...
    await this.modPage.hasElementCount(
      e.chatUserMessageText,
      MESSAGES_COUNT + 2,
      'should display the replied message by the moderator on the mod public chat',
    );
    await this.userPage.hasElementCount(
      e.chatUserMessageText,
      MESSAGES_COUNT + 2,
      'should display the replied message by the moderator on the user public chat',
    );
    await checkLastMessageSent(this.modPage, e.testMessage);
    await checkLastMessageSent(this.userPage, e.testMessage);
    ...
    await expect(
      targetMessageUser,
      'should not display the first message in viewport after multiple messages sent',
    ).not.toBeInViewport();
    await this.userPage.waitAndClick(e.chatMessageReplied);
    await expect(
      targetMessageUser,
      'should display the first message in viewport after clicking on the replied ',
    ).toBeInViewport();
  }
  ...
  async addRemoveReaction() {
    ...
    await this.modPage.waitAndClick(e.reactMessageButton);
    await this.modPage.getByLabelAndClick(e.thumbsUpEmoji);
    const messageReactionsMod = this.modPage.page.locator(e.chatMessageItem).last().locator(e.messageReactionItem);
    const messageReactionsUser = this.userPage.page.locator(e.chatMessageItem).last().locator(e.messageReactionItem);
    ...
    await expect(messageReactionsMod, 'should display the correct reaction item for the mod').toContainText(
      e.thumbsUpEmoji,
    );
    ...
    await expect(
      messageReactionsMod,
      'should not display any reaction item for the mod when all reactions are removed',
    ).not.toBeVisible();
    await expect(
      messageReactionsUser,
      'should not display any reaction item for the viewer when all reactions are removed',
    ).not.toBeVisible();
  }
  ...
  async incrementDecrementReaction() {
    ...
    // hover message
    const lastMessageItemMod = this.modPage.page.locator(e.chatMessageItem).last();
    await lastMessageItemMod.hover();
    await expect(
      lastMessageItemMod.locator(e.messageToolbar),
      'should display the message toolbar when hovering a message',
    ).toBeVisible();
    // click on react button
    await this.modPage.waitAndClick(e.reactMessageButton);
  ...
  async orderReactions() {
    ...
    const lastMessageItemUser = this.userPage.page.locator(e.chatMessageItem).last();
    await lastMessageItemUser.hover();
    await expect(
      lastMessageItemUser.locator(e.messageToolbar),
      'should display the message toolbar when hovering a message',
    ).toBeVisible();
    ...
    await expect(messageReactionsMod.first(), 'should display the first reaction item added for the mod').toContainText(
      e.thumbsUpEmoji,
    );
    await expect(
      messageReactionsUser.first(),
      'should display the first reaction item added for the viewer',
    ).toContainText(e.thumbsUpEmoji);
    // mod increment viewer reaction
    await messageReactionsMod.last().click();
    // check reaction item incremented
    await expect(messageReactionsMod.first(), 'should display the correct reaction count for the mod').toContainText(
      '2',
    );
    await expect(
      messageReactionsUser.first(),
      'should display the correct reaction count for the viewer',
    ).toContainText('2');
    await expect(
      messageReactionsMod.first(),
      'should display the reaction item with the highest amount (grinning face) first for the mod',
    ).toContainText(e.grinningFaceEmoji);
    await expect(
      messageReactionsUser.first(),
      'should display the reaction item with the highest amount (grinning face) first for the viewer',
    ).toContainText(e.grinningFaceEmoji);
  }
}
