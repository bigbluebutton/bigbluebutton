...
export class Options extends MultiUsers {
  ...
  async localesTest() {
    ...
    await Promise.all(
      e.locales.map(async (locale) => {
        ...
        await Promise.all(
          Object.keys(currentValuesBySelector).map(async (selector) => {
  ...
  async darkMode() {
    ...
    expect
      .soft(userListContainerBackgroundColor)
      .not.toEqual(
        await userListContainerLocator.evaluate(getBackgroundColorComputed),
        'should the user list container background color be changed'
      );
    expect
      .soft(whiteboardOptionsButtonBackground)
      .not.toEqual(
        await whiteboardOptionsButtonLocator.evaluate(getBackgroundColorComputed),
        'should the whiteboard options button background color be changed'
      );
    expect
      .soft(sendButtonBackgroundColor)
      .not.toEqual(
        await sendButtonLocator.evaluate(getBackgroundColorComputed),
        'should the send button background color be changed'
      );
    expect
      .soft(joinAudioBackgroundColor)
      .not.toEqual(
        await joinAudioLocator.evaluate(getBackgroundColorComputed),
        'should the join audio button background color be changed'
      );
    expect
      .soft(minimizePresentationBackgroundColor)
      .not.toEqual(
        await minimizePresentationLocator.evaluate(getBackgroundColorComputed),
        'should the minimize presentation button background color be changed'
      );
    expect
      .soft(chatMessagesBackgroundColor)
      .not.toEqual(
        await chatNotificationMessageItemLocator.evaluate(getTextColorComputed),
        'should the chat notification message color be changed'
      );
    expect
      .soft(messageTitleColor)
      .not.toEqual(
        await messageTitleLocator.evaluate(getTextColorComputed),
        'should the message title text color be changed'
      );
    expect
      .soft(presentationTitleColor)
      .not.toEqual(
        await presentationTitleLocator.evaluate(getTextColorComputed),
        'should the presentation title text color be changed'
      );
    expect
      .soft(chatUserMessageTextColor)
      .not.toEqual(
        await chatUserMessageTextLocator.evaluate(getTextColorComputed),
        'should the chat user message text color be changed'
      );
  ...
  async fontSizeTest() {
    ...
    await this.modPage.page.waitForTimeout(500); // wait for the changes to be applied
    expect
      .soft(await presentationTitleLocator.evaluate(getFontSizeNumber))
      .toBeGreaterThan(presentationTitleFontSize, 'should the presentation title font size be increased');
    expect
      .soft(await chatButtonLocator.evaluate(getFontSizeNumber))
      .toBeGreaterThan(chatButtonFontSize, 'should the chat button font size be increased');
    expect
      .soft(await messageTitleLocator.evaluate(getFontSizeNumber))
      .toBeGreaterThan(messageTitleFontSize, 'should the message title font size be increased');
