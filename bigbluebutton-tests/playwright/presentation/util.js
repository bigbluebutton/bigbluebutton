...
import { ELEMENT_WAIT_TIME, UPLOAD_PDF_WAIT_TIME, ELEMENT_WAIT_EXTRA_LONG_TIME } from '../core/constants.ts';
...
export async function uploadSinglePresentation(testPage, fileName, uploadTimeout = UPLOAD_PDF_WAIT_TIME) {
  const firstSlideSrc = await testPage.page.evaluate(
    (selector) => document.querySelector(selector)?.style.backgroundImage.split('"')[1],
    [e.currentSlideImg]
  );
  ...
  await testPage.page.waitForFunction(
    ([selector, previousSlideSrc]) => {
      const currentSrc = document.querySelector(selector)?.style?.backgroundImage?.split('"')[1];
