...
export class Create extends MultiUsers {
  ...
  async create(captureNotes = false, captureWhiteboard = false) {
    ...
    await this.modPage.waitAndClick(e.manageUsers);
    await this.modPage.wasRemoved(e.createBreakoutRooms, 'should not have the "create breakout rooms" option after creating breakout rooms');
    await this.modPage.hasElement(e.inviteUsersToBreakoutRooms, 'should have the "invite to breakout rooms" option when breakout rooms are created');
  }
