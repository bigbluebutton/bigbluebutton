...
enableHTML5ClientLog() {
  ...
  yq-go e -i '.public.clientLog.external.enabled = true' $HTML5_CONFIG
  yq-go e -i ".public.clientLog.external.url = \"$PROTOCOL://$HOST/html5log\"" $HTML5_CONFIG
...
enableUFWRules() {
  ...
  if systemctl is-enabled haproxy> /dev/null 2>&1; then
    if systemctl -q is-active haproxy; then
      ...
      # yq-go e -i '.public.kurento.forceRelayOnFirefox = true' $HTML5_CONFIG
    else
