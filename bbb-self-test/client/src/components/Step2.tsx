...
export const Step2 = (props: Props) => {
    const {checkedList, systemStatus, systemInfos, networkStatus, networkInfos, devicesStatus, devicesInfos } = props;
    ...
    return (
        <List
            ...
            renderItem={(item) => {
                if(item.enabled) {
                    return (
                        <List.Item className='text-left' actions={[<Badge status={getItemStatus(item.statusText)} text={item.statusText} />]}>
