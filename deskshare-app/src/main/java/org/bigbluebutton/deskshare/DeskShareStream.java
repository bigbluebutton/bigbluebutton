...
public class DeskShareStream {
	...
	private BlockingQueue<CaptureEvent> queue = new LinkedBlockingQueue<CaptureEvent>();
	...
	private void handleCaptureEvent(CaptureEvent event) {
		BufferedImage image = event.getCapturedScreen();
		imageReceived(image);
	}
	
	public void accept(CaptureEvent event) {
		try {
			queue.put(event);
		} catch (InterruptedException e) {
