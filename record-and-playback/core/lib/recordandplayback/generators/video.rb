...
module BigBlueButton
  ...
  def BigBlueButton.process_webcam_videos(target_dir, raw_archive_dir, output_width, output_height, output_framerate, audio_offset, processed_audio_file, video_formats=['webm'], show_moderator_viewpoint=false, render_cams_list=["all"], delay_on_closing_cam)
    ...
    render_cams_list.each_with_index do |cam, index|
      ...
      webcams_name = "webcams"
      if index != 0
        webcams_name = "webcams_#{cam}"
      end

      what_to_render = render_cams_list[index] 
      ...
      user_video_file = BigBlueButton::EDL::Video.render(
        user_video_edl, user_video_layout, "#{target_dir}/#{webcams_name}")
      ...
      formats.each do |format|
        filename = BigBlueButton::EDL::encode(
          processed_audio_file, user_video_file, format, "#{target_dir}/#{webcams_name}", audio_offset)
      end
    end
  end
