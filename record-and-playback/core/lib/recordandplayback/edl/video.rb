      ...
      def self.composite_cut(output, cut, layout, videoinfo, id_names, id_fonts)
        ...
        layout[:areas].each do |layout_area|
          ...
          area.each do |video|
            ...
            if id_names[uid]
              border_w = 3
              ffmpeg_filter << ",drawtext=fontfile=#{id_fonts[uid]}:text='#{escapeffmpg(id_names[uid])}':fontsize=#{scale_height/NAME_SIZE_RECIPRCL}:x=#{border_w}:y=#{(tile_height-scale_height)/2+border_w}:box=1:fontcolor=#{NAME_FGCOLOR}:boxcolor=#{NAME_BGCOLOR}@#{NAME_ALPHA}:boxborderw=#{border_w}"
            end
