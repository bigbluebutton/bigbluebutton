      ...
      def self.composite_cut(output, cut, layout, videoinfo, id_names, id_fonts)
        ...
        layout[:areas].each do |layout_area|
          ...
          area.each do |video|
            ...
            if id_names[uid]
              ffmpeg_preprocess_filter << "drawtext=fontfile=#{id_fonts[uid]}:text='#{escapeffmpg(id_names[uid])}':fontsize=#{tile_height/NAME_SIZE_RECIPRCL}:box=1:fontcolor=#{NAME_FGCOLOR}:boxcolor=#{NAME_BGCOLOR}@#{NAME_ALPHA}:boxborderw=3,"
            end
