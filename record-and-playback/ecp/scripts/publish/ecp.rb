...
opts = Trollop::options do
  opt :meeting_id, "Meeting id to archive", :default => 'd9ab062b221350a7b6fcacc6970435c1f651859d-1332487357860-ecp', :type => String
end
...
if (playback == "ecp")
	logger = Logger.new("/var/log/bigbluebutton/ecp/publish-#{meeting_id}.log", 'daily' )
	BigBlueButton.logger = logger
	BigBlueButton.logger.info("Publishing #{meeting_id}")
	# This script lives in scripts/archive/steps while properties.yaml lives in scripts/
	bbb_props = YAML::load(File.open('../../core/scripts/bigbluebutton.yml'))
	simple_props = YAML::load(File.open('ecp.yml'))
	
	recording_dir = bbb_props['recording_dir']
	process_dir = "#{recording_dir}/process/ecp/#{meeting_id}"
	publish_dir = simple_props['publish_dir']
	playback_host = simple_props['playback_host']

	target_dir = "#{recording_dir}/publish/ecp/#{meeting_id}"
	...
	if not FileTest.directory?(target_dir)
		...
		presentation_url = "/ecpslides/" + meeting_id + "/presentation"
