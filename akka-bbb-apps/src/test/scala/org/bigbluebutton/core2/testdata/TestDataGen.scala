...
object TestDataGen {
  def createRegisteredUser(meetingId: String, users: RegisteredUsers, name: String, role: String,
                           guest: Boolean, authed: Boolean, waitForApproval: Boolean, logoutURL: String = ""): RegisteredUser = {
    ...
    val ru = RegisteredUsers.create(meetingId, userId = id, extId, name, role,
      authToken, Vector(sessionToken), avatarURL, webcamBackgroundURL, color, guest, authed, GuestStatus.ALLOW, false, "", logoutUrl, Map(), false)
