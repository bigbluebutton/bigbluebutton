...
trait RequestUserCameraCmdMsgHdlr {
  ...
  def handle(
    ...
    } else if (!liveMeeting.props.usersProp.allowModsToRequestCameraShare) {
      log.info(s"Ignoring camera request, allowModsToRequestCameraShare is disabled. meetingId=${meetingId}")
