...
trait SelectRandomViewerReqMsgHdlr extends RightsManagementTrait {
  ...
  def pickUser(userIds: Vector[String], refresh: Boolean): String = {
    if (userIds.length != 1) { // Note: If there is only one user we do not return null
  ...
  def handleSelectRandomViewerReqMsg(msg: SelectRandomViewerReqMsg): Unit = {
    ...
    def broadcastEvent(msg: SelectRandomViewerReqMsg, choice: String): Unit = {
      ...
      val body = SelectRandomViewerRespMsgBody(msg.header.userId, choice)
    ...
    if (permissionFailed(PermissionCheck.GUEST_LEVEL, PermissionCheck.PRESENTER_LEVEL, liveMeeting.users2x, msg.header.userId)) {
      ...
      val choice = if (allowRepeat) {
        ...
        if (users.size == 0) "null" else userIds(randNum.nextInt(users.size))
      } else pickUser(userIds, refresh)

      broadcastEvent(msg, choice)
    }
  }
}
