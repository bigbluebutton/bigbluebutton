...
import org.bigbluebutton.core.models.{ DisabledFeatures2x, GroupChat, Roles, Users2x }
      ...
      GroupChatApp.getGroupChatOfUsers(msg.header.userId, msg.body.users, state) match {
        ...
        case None =>
          val newState = for {
            ...
            val msgs = msg.body.msg.map(m => GroupChatApp.toGroupChatMessage(createdBy, m, emphasizedText = false, GroupChatMessageType.DEFAULT, false))
            val users = {
              if (msg.body.access == GroupChatAccess.PRIVATE) {
