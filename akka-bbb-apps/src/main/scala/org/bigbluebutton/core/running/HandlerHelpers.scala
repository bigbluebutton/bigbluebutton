...
trait HandlerHelpers extends SystemConfiguration {
  ...
  def sendEndMeetingDueToExpiry(reason: String, eventBus: InternalEventBus, outGW: OutMsgRouter, liveMeeting: LiveMeeting, userId: String, name: String): Unit = {
    endMeeting(outGW, liveMeeting, reason, userId, name)
