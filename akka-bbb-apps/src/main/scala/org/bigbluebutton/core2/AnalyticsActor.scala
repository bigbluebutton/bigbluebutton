...
class AnalyticsActor(val includeChat: Boolean) extends Actor with ActorLogging {
  ...
  def handleBbbCommonEnvCoreMsg(msg: BbbCommonEnvCoreMsg): Unit = {
    ...
    msg.core match {
      ...
      case m: UnmuteRequestAnswerEvtMsg => logMessage(msg)
      ...
      case m: PinGroupChatMessageReqMsg => logChatMessage(msg)
      case m: UnpinGroupChatMessageReqMsg => logChatMessage(msg)
