...
public class ProcessStream implements Runnable {
    ...
    public void run() {
        try {
            log.debug("Creating thread to execute the process stream: {}",this.stream);
            String line;
            ...
            log.debug("IOException on process running...");
            close();
        }

        log.debug("Exiting thread that handles process stream: {}", this.stream);
    }
    ...
    public void close() {
        try {
            if(this.stream != null) {
                log.debug("Closing process stream {}", this.stream);
                this.stream.close();
                this.stream = null;
            }
        }
        catch(IOException ioe) {
            log.debug("IOException on process closing...");
        }
    }
}
