...
// well.
func NewPDFFlow(cfg config.Config, processor presentation.PageProcessor) pipeline.Flow[*presentation.FileToProcess, *presentation.ProcessedFile] {
	...
	generatePages := pipeline.NewStep[*presentation.FileToProcess, *FileWithPages]().Generate(NewPageGeneratorWithProcessorAndConfig(processor, cfg))
