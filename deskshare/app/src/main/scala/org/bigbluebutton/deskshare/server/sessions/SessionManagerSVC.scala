...
class SessionManagerSVC(streamManager: StreamManager, keyFrameInterval: Int, interframeInterval: Int) extends Actor {
	...
	private def createSession(c: CreateSession): Unit = {
		...
		sessions.get(c.room) match {
		  case None => {
			  ...
			  val session: SessionSVC = new SessionSVC(this, c.room, c.screenDim, c.blockDim, streamManager, keyFrameInterval, interframeInterval) 
			  if (session.initMe()) {
