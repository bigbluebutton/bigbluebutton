...
| `userdata-bbb_pre_flight_screen=`              | (Introduced in BigBlueButton 4.0) If set to `true`, shows a pre-flight device selection screen (camera preview with mic, camera and speaker pickers) before joining audio and video, including during guest waiting. Opt-in per join                                                                                             | `false`       |
