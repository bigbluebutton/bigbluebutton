...
To prevent this, make sure to use the [`apply-config.sh` script](https://docs.bigbluebutton.org/admin/customize.html#automatically-apply-configuration-changes-on-restart) to ensure changes are retained upon upgrades and restarts.
...
BigBlueButton uses a TURN server for NAT traversal. By default, a STUN server from freeswitch.org is configured for BigBlueButton. You can [set up your own TURN/STUN server](/admin/setup-turn-server.html), and configure it in `/usr/share/bbb-web/WEB-INF/classes/spring/turn-stun-servers.xml`.
...
By default, when Greenlight creates a conference, the value 'record=true' is passed to BigBlueButton. These conferences will be recorded even if the record button is not pressed. See also: [Recording](https://docs.bigbluebutton.org/admin/privacy.html#bigbluebutton-either-records-all-of-a-session-or-does-not-record-at-all)
...
Greenlight supports displaying of terms and conditions for registered users or upon registration. See <https://docs.bigbluebutton.org/greenlight/gl-config.html#adding-terms-and-conditions>
