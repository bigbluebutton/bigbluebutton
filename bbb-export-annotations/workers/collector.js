...
async function collectSharedNotes(retries = 3) {
  ...
  const nameToSave = `${sanitize(exportJob.nameToSave.replace(/\s/g, '_'))}.${notesFormat}`;
  const notes_endpoint = `${config.bbbPadsAPI}/p/${padId}/export/${notesFormat}`;
  const filePath = path.join(dropbox, nameToSave);
  ...
  const notifier = new WorkerStarter({jobType, jobId, nameToSave, filename: exportJob.filename});
  notifier.notify();
}
