		...
		String source = pres.getUploadedFile().getAbsolutePath();
	 	...
	 	if(SupportedFileTypes.isImageFile(pres.getFileType())){
	 		...
	 		for(int i=1; i<=pres.getNumberOfPages(); i++){
	 			...
	 			COMMAND = IMAGEMAGICK_DIR + "/convert -density 300x300 -resize 800x600 -quality 90 +dither -depth 8 -colors 256 " + File.separatorChar + tmp.getAbsolutePath() + " " + destpng.getAbsolutePath();
	 			done = new ExternalProcessExecutor().exec(COMMAND, 60000);
	 			if(!done){
	 				break;
	 			}
