	...
	private boolean generateThumbnails(File thumbsDir, UploadedPresentation pres) throws InterruptedException {
		...
		if(SupportedFileTypes.isImageFile(pres.getFileType())){
			...
			//COMMAND = IMAGEMAGICK_DIR + "/convert -thumbnail 150x150 " + source + " " + dest;
                        cmdarray = new String[]{IMAGEMAGICK_DIR+File.separator+"convert", "-thumbnail", "150x150", source, dest};
		}else{
			...
			//COMMAND = IMAGEMAGICK_DIR + "/gs -q -sDEVICE=pngalpha -dBATCH -dNOPAUSE -dNOPROMPT -dDOINTERPOLATE -dPDFFitPage -r16 -sOutputFile='" + dest +"%d.png' '" + source+"'";
                        cmdarray = new String[]{IMAGEMAGICK_DIR+File.separator+"gs", "-q", "-sDEVICE=pngalpha", "-dBATCH", "-dNOPAUSE", "-dNOPROMPT", "-dDOINTERPOLATE", "-dPDFFitPage", "-r16", "-sOutputFile="+dest+"%d.png", source};
		}
		...
		if (done) {
			...
			log.warn("Failed to create thumbnails: " + StringUtils.join(cmdarray," "));	 		
		}
