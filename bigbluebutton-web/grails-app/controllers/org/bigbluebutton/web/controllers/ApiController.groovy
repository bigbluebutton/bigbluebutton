...
class ApiController {
    ...
    def uploadDocuments(conf) {
        ...
        String preName = "中软大讲堂.pdf";
        boolean current = true;
        if (requestBody == null) {
            downloadAndProcessDocument(preName, presentationService.defaultUploadedPresentation, conf.getInternalId(), current);
        } else {
            ...
            xml.children().each { module ->
                ...
                if ("presentation".equals(module.@name.toString())) {
                    ...
                    module.children().each { document ->
                        current = "true".equals(document.@current.toString());
                        preName = document.@name.toString();
                        log.info("上传的文档为：" + preName + "\t是否是默认演示文档：" + current);
                        if (!StringUtils.isEmpty(document.@url.toString())) {
                            downloadAndProcessDocument(preName, document.@url.toString(), conf.getInternalId(), current);
                        } else if (!StringUtils.isEmpty(document.@name.toString())) {
                            def b64 = new Base64()
                            def decodedBytes = b64.decode(document.text().getBytes())
                            processDocumentFromRawBytes(decodedBytes, document.@name.toString(), conf.getInternalId(), current);
                        } else {
    ...
    def downloadAndProcessDocument(presFilename, address, meetingId, current) {
        log.info("ApiController#downloadAndProcessDocument(${address}, ${meetingId})");
        if (StringUtils.isEmpty(presFilename)) {
    ...
    def processUploadedFile(meetingId, presId, filename, presFile, current) {
    	log.info("ApiController#processUploadedFile(${meetingId},${presId},${filename},${presFile},${current})")
