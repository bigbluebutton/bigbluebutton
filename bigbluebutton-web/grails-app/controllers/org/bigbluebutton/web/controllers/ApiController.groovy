...
class ApiController {
  ...
  private String decodePresentationUrlFilename(String address) {
    try {
      // Derive the name from the URL path only, dropping any query string or fragment
      // (e.g. presigned S3 links carry ?X-Amz-... parameters that are not part of the filename).
      String urlPath = new URL(address.toString()).getPath()
      return URLDecoder.decode(FilenameUtils.getName(urlPath), URL_ENCODING)
