  ...
  def handleUserJoin(msg: UserJoining):Unit = {
    ...
    regUser foreach { ru =>
      ...
      val uvo = new UserVO(msg.userID, ru.externId, ru.name, 
                  ru.role, ru.guest, waitingForAcceptance=waitingForAcceptance, raiseHand=false, presenter=false, 
      ...
      val user = users.getUserWithVoiceUserId(msg.voiceUser.userId) match {
        ...
        case None => {
          ...
          val uvo = new UserVO(webUserId, webUserId, msg.voiceUser.callerName, 
		                  Role.VIEWER, guest=false, waitingForAcceptance=false, raiseHand=false, presenter=false, 
		                  hasStream=false, locked=false, webcamStream="", 
