...
case "$1" in
  configure|upgrade|1|2)
    ...
    yq-go e -i '.mediasoup.workerBalancing.strategy = "least-loaded"' $TARGET
    ...
    yq-go e -i '.mediasoup.enableWorkerTransposing = true' $TARGET
    ...
    yq-go e -i '.mediasoup.dedicatedMediaTypeWorkers.audio = "auto"' $TARGET
    ...
    if [ "$FREESWITCH_IP" != "" ]; then
      yq-go e -i ".freeswitch.ip = \"$FREESWITCH_IP\"" $TARGET
    ...
    yq-go e -i '.livekit.enabled = true' $TARGET
    ...
    if [ -f /etc/bigbluebutton/livekit.yaml ]; then
      ...
      if [ -n "$LIVEKIT_KEY" ] && [ "$LIVEKIT_KEY" != "null" ] \
        ...
        yq-go e -i ".livekit.key = \"$LIVEKIT_KEY\"" $TARGET
        yq-go e -i ".livekit.secret = \"$LIVEKIT_SECRET\"" $TARGET
      fi
    fi
    set -x
