...
case "$1" in
  configure|upgrade|1|2)
    ...
    yq-go e -i  ".bbb.sharedSecret  = \"$BBB_SECRET\"" $TARGET
    yq-go e -i  ".bbb.serverDomain = \"$BBB_HOST\"" $TARGET
    yq-go e -i  '.bbb.auth2_0 = true' $TARGET
    yq-go e -i  '.modules."../out/webhooks/index.js".config.getRaw = false' $TARGET
    yq-go e -i  '.log.filename = "/var/log/bbb-webhooks/bbb-webhooks.log"' $TARGET
