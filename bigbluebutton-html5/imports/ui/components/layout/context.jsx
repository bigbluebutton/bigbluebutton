...
import {
  getPluginUnpinnedApps,
  addPluginUnpinnedApp,
  ...
  switch (action.type) {
    ...
    case ACTIONS.SET_SIDEBAR_NAVIGATION_PIN_APP: {
      ...
      const {
        id: appId,
        pin,
        // Native apps that should be initially pinned (e.g. timer, poll, breakouts)
        isDefault = false,
        // Plugin injected items in the apps gallery that should be initially pinned.
      ...
      // Stable key across sessions: appId includes a per-session UUID generated by the plugin.
      // Using pluginName:name is stable even when the same plugin injects multiple panels,
      // since each panel has a distinct display name.
      const pluginAppKey = registeredApps[appId]?.pluginName && registeredApps[appId]?.name
        ? `${registeredApps[appId].pluginName}:${registeredApps[appId].name}`
        : null;

      // Plugin default: skip if user already explicitly unpinned this app.
      // On first load or after user re-pins, behaves like a regular user pin (allows replacement).
      if (
        isPluginDefault
        && pin
        && pluginAppKey
        && getPluginUnpinnedApps().includes(pluginAppKey)
      ) return state;

      // Track user-initiated unpins of plugin apps to prevent re-pinning on reload.
      if (!pin && pluginAppKey) {
        addPluginUnpinnedApp(pluginAppKey);
      }

      // If user manually re-pins a plugin app, clear the unpin memory.
      if (pin && !isDefault && !isPluginDefault && pluginAppKey) {
        removePluginUnpinnedApp(pluginAppKey);
      }
