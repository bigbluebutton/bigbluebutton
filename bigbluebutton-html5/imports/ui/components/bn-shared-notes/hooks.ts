...
function useFlipCursorWhenTopPosition() {
  const editorRef = useRef<HTMLDivElement>(null);

  // Monitor cursor positions and flip labels when near top edge
  useEffect(() => {
    ...
    const checkCursorPositions = () => {
      ...
      const scrollContainer = document.getElementById('bn-notes-scroll-container');
      if (!scrollContainer) return;

      const cursors = editorElement.querySelectorAll('.bn-collaboration-cursor__caret');
      const containerRect = scrollContainer.getBoundingClientRect();
      const thresholdRem = 3; // rem units from top to trigger flip
      const rootFontSize = parseFloat(getComputedStyle(document.documentElement).fontSize);
      const threshold = thresholdRem * rootFontSize;

      cursors.forEach((cursor) => {
        const cursorRect = cursor.getBoundingClientRect();
        const distanceFromTop = cursorRect.top - containerRect.top;

        if (distanceFromTop < threshold) {
          cursor.classList.add('near-top');
        } else {
          cursor.classList.remove('near-top');
        }
      });
    };

    // Get scroll container for event listener
    const scrollContainer = document.getElementById('bn-notes-scroll-container');

    const interval = setInterval(checkCursorPositions, 100);
    scrollContainer?.addEventListener('scroll', checkCursorPositions);

    return () => {
      clearInterval(interval);
      scrollContainer?.removeEventListener('scroll', checkCursorPositions);
    };
  }, []);
  return editorRef;
}

export { useHocuspocusProvider, useBlockNoteLocaleLanguage, useFlipCursorWhenTopPosition };
