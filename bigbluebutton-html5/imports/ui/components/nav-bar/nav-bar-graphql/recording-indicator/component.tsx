...
const RecordingIndicator: React.FC<RecordingIndicatorProps> = ({
  ...
  const setIntervalRef = React.useRef<any>(null);

  const recordingToggle = useCallback((micUser, recording) => {
  ...
  const title = useMemo(() => {
    return intl.formatMessage(recording ? intlMessages.recordingIndicatorOn
  ...
  const recordMeetingButton = useMemo(() => {
    ...
    return (
      <Styled.RecordingControl
        ...
        onClick={() => { recordingToggle(micUser, recording) }}
        onKeyPress={() => { recordingToggle(micUser, recording) }}
      >
  ...
  return (
    <>
      ...
      {isRecordingNotifyModalOpen ? <RecordingNotify
        toggleShouldNotify={() => { setShouldNotify(prev => !prev) }}
        priority="high"
...
const RecordingIndicatorContainer: React.FC = () => {
  ...
  if (meetingRecordingPoliciesError || meetingRecordingError) return (
    <div>
  ...
  return <RecordingIndicator
    ...
    isPhone={isMobile}
    recordingNotificationEnabled={((meetingRecording?.startedBy !== currentUser?.userId) && currentMeeting?.notifyRecordingIsOn) ?? false}
