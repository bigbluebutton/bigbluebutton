...
type handler struct {
	flow pipeline.Flow[*http.Request, *meeting.CreateMeetingResponse]
}
...
func (h *handler) Handle(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		errResp := meeting.ToResponse(err)
...
// to handle incoming CreateMeeting requests.
func NewHandlerFunc(flow pipeline.Flow[*http.Request, *meeting.CreateMeetingResponse]) http.HandlerFunc {
	h := &handler{flow}
	return h.Handle
}
