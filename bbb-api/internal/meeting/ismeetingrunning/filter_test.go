...
func TestRequestFilter_Filter(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:          "Valid checksum and valid meeting ID",
		...
		{
			name:          "Valid checksum but missing meeting ID",
			meetingID:     "",
			includeParams: true,
			setupRequest: func() *http.Request {
				endpoint := "isMeetingRunning"
		...
		},
		{
			...
			setupRequest: func() *http.Request {
				...
				req := httptest.NewRequest(http.MethodGet, "/isMeetingRunning?meetingID="+encodedMeetingID+"&checksum="+checksum, nil)
